...
export const MemoizedMasonryCard = memo(function MemoizedMasonryCard({ customFolders, customLists, onMoveToFolder, onMoveToList, onUpdateTags, onTagClick, item, theme, isDark, activeWorkspace, currentUserId, teamRole, viewMode, onClick, inTrash, onRestore, onHardDelete, onDelete, onUpdateSticky, toggleItemReaction, toggleChecklistItem, isSelected, onToggleSelect, isSelectMode, onPlayYouTube, teamMembers }: any) {
  ...
  return (
    <motion.div 
      ...
      className={`group/card relative rounded-2xl transition-all duration-500 flex flex-col w-full border ${theme.card} ${theme.cardHover} ${itemType === 'note' && !inTrash ? 'cursor-text' : inTrash ? 'cursor-default' : 'cursor-pointer'} font-sans ${viewMode === 'card' ? 'h-85' : 'h-full'} ${isSelected ? 'ring-4 ring-teal-500 scale-[0.98]' : ''} ${isCardMenuOpen || isTagMenuOpen ? 'z-30 shadow-2xl' : 'z-10 hover:z-20'} ${!inTrash && !isSelectMode ? 'active:cursor-grabbing hover:cursor-grab' : ''}`}
    >
      {canModify && (
         <div className="absolute top-3 left-3 z-40 pointer-events-auto">
            <button 
               ...
               className={`p-1.5 rounded-full transition-all border shadow-md backdrop-blur-xl active:scale-95 ${isSelected ? 'opacity-100 bg-teal-500 border-teal-600 text-white' : `opacity-0 group-hover/card:opacity-100 ${isDark ? 'bg-[#1C1C1E]/80 border-white/20 text-white hover:bg-black' : 'bg-white/90 border-black/10 text-stone-800 hover:bg-white'}` }`}
            >
        ...
        <div className="absolute top-0 left-0 w-full z-40" ref={controlsRef}>
           <div className="absolute top-3 right-3 flex items-center gap-1.5 pointer-events-auto">
               <button 
                  ...
                  className={`p-2 rounded-full opacity-0 group-hover/card:opacity-100 transition-all border shadow-md backdrop-blur-xl active:scale-95 ${isTagMenuOpen ? 'opacity-100' : ''} ${isDark ? 'bg-[#1C1C1E]/90 border-white/10 text-white hover:bg-black' : 'bg-white/95 border-black/5 text-stone-800 hover:bg-white'}`}
               >
               ...
               <button 
                  ...
                  className={`p-2 rounded-full opacity-0 group-hover/card:opacity-100 transition-all border shadow-md backdrop-blur-xl active:scale-95 ${isCardMenuOpen ? 'opacity-100' : ''} ${isDark ? 'bg-[#1C1C1E]/90 border-white/10 text-white hover:bg-black' : 'bg-white/95 border-black/5 text-stone-800 hover:bg-white'}`}
               >
           ...
           <AnimatePresence>
              {isTagMenuOpen && (
                 <motion.div 
                    ...
                    className={`absolute top-14 left-3 right-3 origin-top z-50 rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.2)] border p-3 backdrop-blur-3xl pointer-events-auto ${isDark ? 'bg-[#1C1C1E]/95 border-white/10' : 'bg-white/95 border-stone-200'}`}
           ...
           <AnimatePresence>
              {isCardMenuOpen && (
                 <motion.div 
                    initial={{ opacity: 0, scale: 0.95, y: -5 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: -5 }} transition={{ type: "spring", stiffness: 400, damping: 25 }}
                    className={`absolute top-14 left-3 right-3 origin-top z-50 rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.2)] border p-1.5 backdrop-blur-3xl pointer-events-auto flex flex-col ${isDark ? 'bg-[#1C1C1E]/95 border-white/10' : 'bg-white/95 border-stone-200'}`}
                    onClick={e => e.stopPropagation()}
                 >
                    {menuView === 'main' ? (
                       <>
                          ...
                          {!stickyText ? (
                             <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); setIsEditingSticky(true); setIsCardMenuOpen(false); }} className={`w-full text-left px-3 py-2.5 text-xs font-bold rounded-xl flex items-center gap-2.5 transition-colors ${isDark ? 'hover:bg-white/10 text-zinc-300' : 'hover:bg-stone-100 text-stone-700'}`}>
                                <FileText size={14} strokeWidth={2.5} className="text-stone-400 dark:text-zinc-500" /> Add Sticky Note
                             </button>
                          ) : (
                             <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); setStickyText(""); onUpdateSticky(item.id, ""); setIsCardMenuOpen(false); }} className={`w-full text-left px-3 py-2.5 text-xs font-bold rounded-xl flex items-center gap-2.5 transition-colors ${isDark ? 'hover:bg-white/10 text-zinc-300' : 'hover:bg-stone-100 text-stone-700'}`}>
                                <X size={14} strokeWidth={2.5} className="text-stone-400 dark:text-zinc-500" /> Remove Sticky
                             </button>
                          )}
                       ...
                       <div className="flex flex-col max-h-56 overflow-y-auto custom-scrollbar">
                          ...
                          <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); onMoveToFolder(item.id, ""); setIsCardMenuOpen(false); }} className={`flex items-center justify-between px-3 py-2.5 text-xs font-bold rounded-xl transition-colors ${isDark ? 'hover:bg-white/10 text-zinc-300' : 'hover:bg-stone-100 text-stone-700'}`}>
                             None
                             {!item.section && !item.sections?.[0] && <Check size={14} strokeWidth={3} className="text-teal-500" />}
                          </button>
                          {customFolders?.map((f: string) => (
                             <button key={f} onClick={(e) => { e.preventDefault(); e.stopPropagation(); onMoveToFolder(item.id, f); setIsCardMenuOpen(false); }} className={`flex items-center justify-between px-3 py-2.5 text-xs font-bold rounded-xl transition-colors ${isDark ? 'hover:bg-white/10 text-zinc-300' : 'hover:bg-stone-100 text-stone-700'}`}>
                       ...
                       <div className="flex flex-col max-h-56 overflow-y-auto custom-scrollbar">
                          ...
                          <div className={`w-full h-px mb-1 ${isDark ? 'bg-white/10' : 'bg-black/5'}`} />
                          <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); onMoveToList(item.id, ""); setIsCardMenuOpen(false); }} className={`flex items-center justify-between px-3 py-2.5 text-xs font-bold rounded-xl transition-colors ${isDark ? 'hover:bg-white/10 text-zinc-300' : 'hover:bg-stone-100 text-stone-700'}`}>
                             None
                             {!item.list_name && <Check size={14} strokeWidth={3} className="text-teal-500" />}
                          </button>
                          {customLists?.map((l: string) => (
                             <button key={l} onClick={(e) => { e.preventDefault(); e.stopPropagation(); onMoveToList(item.id, l); setIsCardMenuOpen(false); }} className={`flex items-center justify-between px-3 py-2.5 text-xs font-bold rounded-xl transition-colors ${isDark ? 'hover:bg-white/10 text-zinc-300' : 'hover:bg-stone-100 text-stone-700'}`}>
      ...
      <div className={`flex flex-col w-full h-full rounded-2xl overflow-hidden relative z-0 ${isSelected ? 'opacity-80' : ''}`}>
        ...
        {(!inTrash && (stickyText || isEditingSticky)) && (
            <div 
              ...
              // --- FIX: Forced into a perfect 24x24 square, locked top-left ---
              className={`absolute top-3 left-3 z-30 w-24 h-24 bg-red-500/95 backdrop-blur-md text-white p-2.5 shadow-xl font-sans text-[10px] font-medium leading-relaxed flex flex-col items-center justify-center text-center rounded-xl pointer-events-auto border border-red-400 overflow-hidden ${canModify && !isSelectMode ? 'cursor-text hover:scale-105 hover:bg-red-500' : 'cursor-default'} transition-all`}
            >
                {isEditingSticky ? (
                    <textarea 
                       ref={textareaRef} autoFocus value={stickyText} onChange={handleStickyChange} onBlur={handleStickyBlur}
                       className="w-full h-full bg-transparent text-white outline-none resize-none placeholder:text-white/70 overflow-hidden text-center"
                       placeholder="Note..."
                    />
                ) : (
        ...
        {itemType === "image" || itemType === "video" || itemType === "audio" || itemType === "document" ? (
          <div className={`w-full relative font-sans flex-1 flex flex-col justify-between ${isDark ? 'bg-[#121214]' : 'bg-stone-50'} ${viewMode === 'card' ? 'h-full' : 'h-auto'}`}>
            {itemType === "video" && !item.url ? ( 
              <video src={displayImg} muted autoPlay loop playsInline draggable={false} className={`w-full object-cover transition-transform duration-700 group-hover/card:scale-105 pointer-events-none ${viewMode === 'card' ? 'h-full' : 'h-auto'}`} />
            ) : displayImg ? (
              <img src={displayImg} loading="lazy" draggable={false} alt={item.title || "Media"} className={`w-full object-cover transition-transform duration-700 group-hover/card:scale-105 ${viewMode === 'card' ? 'h-full' : 'h-auto'} pointer-events-none`} />
            ) : (
                   ...
                   <>
                     ...
                     {activeWorkspace === 'team' && item.creator && (
                       <p className="text-white/80 text-[10px] mt-3 flex items-center gap-1.5 font-bold uppercase tracking-widest [text-shadow:0_1px_2px_rgba(0,0,0,0.8)]">
                         <img src={item.creator_avatar || `https://api.dicebear.com/9.x/shapes/svg?seed=${item.creator}`} loading="lazy" draggable={false} className="w-4 h-4 rounded-full bg-white/20 shadow-sm" />
                ...
                return (
                  <div className={`w-full relative shrink-0 border-b ${isDark ? 'border-zinc-800/50' : 'border-stone-200'} ${mediaHeightClass}`}>
                    <div className="absolute inset-0 overflow-hidden">
                       <img src={displayImg} loading="lazy" draggable={false} className="w-full h-full object-cover group-hover/card:scale-105 transition-transform duration-700 pointer-events-none" />
                    </div>
                  </div>
                );
              })()
              ...
              <div className="z-10 mt-auto w-full">
                ...
                {itemType === 'social_video' ? (
                   <p className={`text-[10px] flex items-center gap-1.5 truncate font-bold uppercase tracking-widest ${theme.textMuted}`}>
                      {item.content || "YouTube Video"} {item.content && item.list_name ? '•' : ''} {item.list_name}
                   </p>
                ) : (
                   <p className={`text-[10px] flex items-center gap-1.5 truncate font-bold uppercase tracking-widest ${theme.textMuted}`}>
                      {item.url}
                ...
                {activeWorkspace === 'team' && item.creator && (itemType === 'link' || itemType === 'social_video') && (
                   <p className={`text-[10px] flex items-center gap-1.5 truncate font-bold uppercase tracking-widest mt-3 pt-3 border-t ${isDark ? 'border-white/5 text-zinc-400' : 'border-stone-200 text-stone-500'}`}>
                      <img src={item.creator_avatar || `https://api.dicebear.com/9.x/shapes/svg?seed=${item.creator}`} loading="lazy" draggable={false} className="w-4 h-4 rounded-full shrink-0" />
            ...
            <div className="mt-auto pt-4 relative z-10 shrink-0 w-full">
              {activeWorkspace === 'team' && item.creator && (
                <div className={`flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest ${theme.textMuted}`}>
                   <img src={item.creator_avatar || `https://api.dicebear.com/9.x/shapes/svg?seed=${item.creator}`} loading="lazy" draggable={false} className="w-4 h-4 rounded-full shadow-sm shrink-0" />
                   {cleanName(item.creator)}
                </div>
              )}
            </div>
              ...
              {activeReactions.map(([emoji, users]: any) => {
                 ...
                 return (
                   <div key={emoji} className="relative group/reactionpill">
                       ...
                       <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-black/90 text-white text-[9px] rounded-lg opacity-0 group-hover/reactionpill:opacity-100 pointer-events-none whitespace-nowrap z-100 transition-opacity">
                          {tooltipText}
                       </div>
                   </div>
                 )
        ...
        {item.tags && item.tags.length > 0 && (
           <div className="flex flex-wrap gap-2 px-4 sm:px-5 pb-4 sm:pb-5 relative z-20 shrink-0 pointer-events-auto">
              {item.tags.map((t: string) => (
                  <button 
                     key={t} 
                     disabled={isSelectMode}
                     onClick={(e) => { e.stopPropagation(); if(!isSelectMode) onTagClick(t); }} 
                     className={`px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest transition-transform ${isSelectMode ? 'opacity-50 cursor-default' : 'hover:scale-105 active:scale-95'} ${isDark ? 'bg-white/10 text-white/70 hover:bg-white/20 hover:text-white' : 'bg-stone-200 text-stone-700 hover:bg-stone-300 hover:text-stone-900'}`}
                  >
                     #{t}
                  </button>
              ))}
